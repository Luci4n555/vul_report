...
//        }
    ...
    public function get_user_info($user_id)
    {
        ...
        $sql = "select status from ecs_cancellation where user_id = '" . $user_id . "'";
        $cancellation_status = $this->getORM()->queryRow($sql);
        ...
        $return = array(
            ...
            'cancellation_status' => $cancellation_status['status'],
        );
    ...
    public function getUserRank($user_id)
    {
        ...
        // 获取当前的会员等级信息
        $rank_sql = "select rank_name,discount from ecs_user_rank where min_points <= '" . $rank['rank_points'] . "' 
        and max_points > '" . $rank['rank_points'] . "'";
    ...
    {
        $sql = "select val from ecs_app_config where k = 'login_logo'";
        $result = $this->getORM()->queryRow($sql);
        return empty($result['val']) ? "https://imgt1.oss-cn-shanghai.aliyuncs.com/ecAllRes/images/logo.png" : goods_img_url($result['val']);
    }
    ...
    public function getAviBonusList($user_id, $page, $status, $bonus_id)
    {
        $thistime = time();
        ...
        if ($status == 'showinvalid') {
            ...
            $data = $this->getORM()->queryRows($sql);
            foreach ($data as $key => $item) {
                if($data[$key]['bonus_id'] == $bonus_id){
                    $data[$key]['bonus_status'] = 1;
                }else{
                    $data[$key]['bonus_status'] = 0;
                }
                $data[$key]['use_end_date'] = date('Y-m-d', $item['use_end_date']);
                $data[$key]['use_start_date'] = date('Y-m-d', $item['use_start_date']);
            }
    ...
    public function nextBingdingMobileModel($openid, $platform, $unionid,$nickname)
    {
        //查询是否已经注册，user_name
        ...
        if ($data['user_name'] == '' || empty($data['user_name'])) {
            ...
            if ($get_wechat['res'] == false) {
                ...
                $orm = $this->getORM();
                $data['user_name'] = $this->filterEmoji($data['user_name']);
    ...
    public function getSettingModel($user_id)
    {
        ...
        $data['mobile'] = $this->getORM()->queryRow($sql);
        $sql = "SELECT status FROM ecs_cancellation WHERE user_id = '$user_id'";
        $data['cancellation_status'] = $this->getORM()->queryRow($sql);
        return $data;
    }
    ...
    public function bindWXH5($nickname, $openid, $randpwd, $unionid)
    {
        ...
        $status = $this->getORM()->executeSql($sql);
...
//        var_dump($chartcode == $chart_code);die;
    ...
    public function getQq()
    {
        ...
        $fix_qq = explode(',', $qq['value']);
        $count_qq = count($fix_qq);
        $rand = rand(0, $count_qq);
        return $fix_qq[$rand];
    }
     ...
     * @return int
     */
    public function checkWxmpLogoModel($user_id)
    {
        $sql = "select user_pic from ecs_users where user_id = '$user_id'";
        $data = $this->getORM()->queryRow($sql);
        return $data;
    }
     ...
     * @return int
     */
    public function insertWxmpLogoModel($user_id, $image_url, $nickname)
    {
        $sql = "update ecs_users set user_pic = '" . $image_url . "',nickname = '" . $nickname . "' where user_id = " . $user_id . " ";
        $data = $this->getORM()->queryRow($sql);
        return $data;
    }
     ...
     * @return int
     */
    public function selectMemberMpLogoModel($user_id)
    {
        $sql = "select user_pic,nickname from ecs_users where user_id = '$user_id'";
        $data = $this->getORM()->queryRow($sql);
        return $data;
    }
