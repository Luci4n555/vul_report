...
class Brand extends NotORM
{
    ...
    {
        // 返回首页展示品牌
        $data = $this->getORM()->where("is_index", 1)->where("is_show",1)->limit(4)->order("sort_order")->fetchAll();
        foreach ($data as $k => $v) {
    ...
    public function getBrandDetailAction($page, $id, $user_id)
    {
        ...
        $sql = "SELECT goods_id,goods_name AS name,shop_price AS retail_price,original_img , goods_thumb AS list_pic_url FROM ecs_goods WHERE brand_id = {$id}  AND is_on_sale = 1 andis_delete = 0 LIMIT {$offset},{$page_size}";
