...
class Category extends NotORM
{
    ...
    public function getCategoryListGoods($cat_id, $page, $order, $type, $user_id)
    {
        ...
        $sql = "SELECT cat_id FROM ecs_category  WHERE parent_id = {$cat_id} OR cat_id ={$cat_id}";

        $category_list = $this->getORM()->queryRows($sql);
        $category_id = $this->getcategory($category_list);


        $sql = "SELECT goods_id FROM ecs_goods_cat  WHERE cat_id = {$cat_id}";
        $category_list = $this->getORM()->queryRows($sql);

        foreach ($category_list as $k => $v) {
            $str[] = $v['goods_id'];
        }
        $goods_id = implode(',', $str);
        if ($goods_id) {
        ...
        FROM ecs_goods 
        WHERE 
        goods_id in ({$goods_id}) OR cat_id in ({$category_id}) and is_on_sale = 1 and is_delete =0 and goods_number>0 order by {$sort} {$order} LIMIT {$offset},{$page_size}";
        } else {
            $sql = "SELECT goods_id AS id,goods_thumb AS list_pic_url,goods_name AS name,shop_price AS retail_price,market_price,virtual_sales 
        FROM ecs_goods 
        WHERE  cat_id in ({$category_id}) and is_on_sale = 1 and is_delete =0 order by {$sort} {$order} LIMIT {$offset},{$page_size}";
        }
...
//        FROM ecs_goods
//        WHERE
//        cat_id in ({$category_id}) and is_on_sale = 1 and is_delete =0 and goods_number>0 order by {$sort} {$order} LIMIT {$offset},{$page_size}";
//        $goods_list = $this->getORM()->queryRows($sql);
        $show_marketprice = $Shop_config_model->get_show_marketprice();
//        $gid = '0';
        if (!empty($goods_list)) {
            $gid = $this->getcategoryid($goods_list);
            ...
            foreach ($goods_list as $k => $v) {
                ...
                $goods_list[$k]['market_price'] = $this->goods_model->price_format($goods_list[$k]['market_price']);
                $goods_list[$k]['shop_price'] = $this->goods_model->get_price($goods_list[$k]['id'],$user_id);
                $goods_list[$k]['retail_price'] = $this->goods_model->price_format($goods_list[$k]['retail_price']);
            }
        }
//        $sql = "SELECT * FROM ecs_goods_cat WHERE cat_id in ({$cat_id})";
...
//                        FROM ecs_goods
//                        WHERE
//                        goods_id = '" . $value['goods_id'] . "' and goods_id not in ({$gid}) and is_delete = 0 order by {$sort} {$order} LIMIT {$offset},{$page_size}";
//
...
//        }
        ...
        $sql = "SELECT cat_id AS id,cat_name AS name,cat_desc AS front_desc,parent_id FROM ecs_category WHERE cat_id in ({$cat_id}) and is_show = 1";
        $now_nav = $this->getORM()->queryRows($sql);


        $sql = "SELECT cat_id FROM ecs_category WHERE parent_id in ({$cat_id}) and is_show = 1";
        $parent_id = $this->getORM()->queryRows($sql);
        $pid = array_column($parent_id,"cat_id");
        $parent = implode(",",$pid);

        if($parent!==''){
            $sql = "SELECT COUNT(goods_id) AS num FROM ecs_goods WHERE cat_id in ({$cat_id},{$parent})";
            $page_total = $this->getORM()->queryRows($sql);
        }else{
            $sql = "SELECT COUNT(goods_id) AS num FROM ecs_goods WHERE cat_id in ({$cat_id})";
            $page_total = $this->getORM()->queryRows($sql);
        }
        return ['currentNav' => $now_nav[0], "data" => $goods_list, "pagetotal" => ceil($page_total[0]['num'] / $page_size)];
    }

    //分割字符串，拼接分类查询sql需要的条件
    ...
    public function getIndexTop()
    {
       ...
       // $data = $this->getORM()->where("is_top", 1)->select("cat_id as id,cat_name,cate_img")->limit(11)->fetchAll();
        $data = $this->getORM()->where("is_top", 1)->select("cat_id as id,cat_name,cate_img")->order('cat_id')->limit(10)->fetchAll();
        foreach ($data as $k => $v) {
