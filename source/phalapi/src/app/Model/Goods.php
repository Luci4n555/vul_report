...
use App\Lib\GiftBag as giftbag;
...
//        $goods_data['shop_price'] = $goods_data['spike_sum'];
        ...
        $goods_data['promote_price'] = $this->price_format($goods_data['promote_price']);
        $goods_data['market_price'] = $this->price_format($goods_data['market_price']);
...
//            $fmt = "%s:%s[%s] \n";
    ...
    public function getNewGoods($user_id)
    {
        ...
        foreach ($data as $k => $v) {
            ...
            $data[$k]["market_price"] = $this->price_format($data[$k]["market_price"]);
			//$data[$k]['shop_price'] = $this->get_price($v['goods_id'], $user_id);
    ...
    public  function integralgoods($page)
    {
        ...
        $offset = ($page - 1) * $page_size;
        /* 获取数据 */
        $sql = 'SELECT eg.* , g.goods_name,g.market_price,g.virtual_sales,g.goods_thumb as list_pic_url ' .
    ...
    public function GetSuperPackageApi($page)
    {
        ...
        foreach ($data as $k => $v) {
            ...
            $giftbag_num = $giftbag->whereGoodsId($v['act_id']);
            $shop_price_data = [];
            foreach ($giftbag_num as $k1 => $v1) {
                $shop_price_data[] = $v1['shop_price'];
            }
            $data[$k]['shop_price'] = array_sum($shop_price_data);
        }
    ...
    public function is_miaoshaa($goods_id)
    {
        $sql = "select is_promote from ecs_goods where goods_id = '$goods_id'";
        $result = $this->getORM()->queryRow($sql);

        return $result;
    }

    public function is_miaoshajia($goods_id)
    {
        $sql = "select promote_price from ecs_goods where goods_id = '$goods_id'";
        $result = $this->getORM()->queryRow($sql);
        return $result;
    }
    ...
    public function get_price($goods_id, $user_id)
    {
        ...
        if (isset($user_id) && !empty($user_id)) {
            $sql = "SELECT user_rank,user_id,rank_points FROM ecs_users WHERE user_id = '$user_id'";
            $res = $this->getORM()->queryRow($sql);
            $user_rank = $res['user_rank'];
            $rank_points = $res['rank_points'];
...
//            }
            ...
            // 获得当前用户等级等级折扣率
            $userRankSql = "SELECT discount FROM ecs_user_rank WHERE min_points <= '$rank_points' and max_points >= '$rank_points'";
            $user_rank_msg = $this->getORM()->queryRow($userRankSql);
            $price = $this->price_format($goods_data['shop_price']);
...
//            }
    ...
    public function getPtGoodsList($page = 1)
    {
        $page_size = 12;
        $offset = ($page - 1) * $page_size;
        $sql = "SELECT goods_id,goods_name,shop_price,virtual_sales,market_price,goods_thumb as list_pic_url,pt_price FROM ecs_goods WHERE is_pintuan = '1' LIMIT {$offset},{$page_size}";
     ...
     * 首页新品跳转的页面，取得所有新品
     */
    public function getSpikeGoodsList($page = 1,$user_id)
    {
        $page_size = 12;
        $offset = ($page - 1) * $page_size;
        $sql = "SELECT goods_id,goods_name,shop_price,virtual_sales,market_price,goods_thumb as list_pic_url,goods_number FROM ecs_goods WHERE is_new = '1' and is_delete ='0' LIMIT {$offset},{$page_size}";
        $res = $this->getORM()->queryAll($sql);

        foreach ($res as $k => $v) {
            $res[$k]['market_price'] = $this->price_format($res[$k]['market_price']);
            $res[$k]['shop_price'] = $this->get_price($res[$k]['goods_id'], $user_id);
            $res[$k]['retail_price'] = $this->price_format($res[$k]['retail_price']);
            $res[$k]['list_pic_url'] = empty($res[$k]['list_pic_url']) ? default_category_goodsImage() : goods_img_url($res[$k]['list_pic_url']);
        }
     ...
     * 人气推荐的商品，用做首页跳转
     */
    public function gethotGoodsList($page = 1,$user_id)
    {
        $page_size = 12;
        $offset = ($page - 1) * $page_size;
        $sql = "SELECT goods_id,goods_name,shop_price,virtual_sales,market_price,goods_thumb as list_pic_url,goods_number FROM ecs_goods WHERE is_hot = '1' and is_delete ='0' LIMIT {$offset},{$page_size}";
        $res = $this->getORM()->queryAll($sql);
    ...
    public function getMiaoShaGoodsList($page = 1)
    {
        $page_size = 12;
        $offset = ($page - 1) * $page_size;
        $sql = "SELECT goods_id,goods_name,shop_price,spike_sum,virtual_sales,market_price,goods_thumb as list_pic_url,goods_number FROM ecs_goods WHERE active = 'true' LIMIT {$offset},{$page_size}";
        $res = $this->getORM()->queryAll($sql);
