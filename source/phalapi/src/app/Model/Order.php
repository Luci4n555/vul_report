...
class Order extends NotORM
{
    ...
    function gen_order($user_id, $address_id, $shipping_id, $bonus_id = '', $point = 0, $tax_data = [], $pack_name, $pack_fee, $ral, $integralGoodsid, $o_time, $super, $superGoodsid, $delivery_id, $delivery_name, $result_spike, $goods_id, $p_type, $pintuan, $pintuanGoodsId, $pintuanNumber, $productId, $pintuanOrderId, $order_amount, $msg,$sku,$platform)
    {
        ...
        if ($bonus_id != '') {
            ...
            if (bcsub($order_amount, $res['type_money']) > 0) {
                $this->msg = '红包错误/被使用';
                return false;
            }
        }
        $order_data = $this->get_order_data($user_id, $address_id, $shipping_id, $bonus_id, $point, $pack_name, $pack_fee, $ral, $integralGoodsid, $o_time, $super, $superGoodsid, $delivery_id, $delivery_name, $result_spike, $goods_id, $p_type, $pintuan, $pintuanGoodsId, $pintuanNumber, $pintuanOrderId, $msg,$platform);
        ...
        if ($ral != 'true' && $super != 'true' && $result_spike != 'true' && $pintuan != 'true') {
            ...
            $skku = explode(",", $sku);
            $char = implode("','", $skku);
            $goods_attr = $this->goods_model->get_goods_attr_info(array($char));
            $insert = "insert into ecs_order_goods(order_id, goods_id, goods_name, goods_sn, product_id, goods_number, market_price,goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id) value
 ('$order_id','$goods_id','$goods_name','$goods_sn','0','1','$goods_price','$goods_number','$goods_attr','$goods_is_real','$goods_extension_code','0','0',' ')";
            ...
            $sql = "INSERT INTO ecs_order_goods SET order_id = '$order_id',
                  ...
                  is_real = 1, goods_attr_id = '$goods_attr_id', goods_attr = '$goods_attr'
                  ";
    ...
    function get_order_data($user_id, $address_id, $shipping_id, $bonus_id, $point, $pack_name, $pack_fee, $ral, $integralGoodsid, $o_time, $super, $superGoodsid, $delivery_id, $delivery_name, $result_spike, $goods_id, $p_type, $pintuan, $pintuanGoodsId, $pintuanNumber, $pintuanOrderId, $msg,$platform)
    {
    ...
    function order_fee($order, $user_id, $consignee, $shipping_id, $ral, $integralGoodsid, $super, $superGoodsid, $result_spike, $goods_id, $pintuan, $pintuanGoodsId, $pintuanNumber)
    {
        ...
        if ($shipping_id > 0 && $total['real_goods_count'] > 0) {
            ...
            if (!empty($shipping_info)) {
                ...
                $sql = 'SELECT count(*) as allcount FROM ecs_cart a,ecs_goods b' . " WHERE  a.user_id = '" . $user_id . "' AND a.extension_code != 'package_buy'
                 AND b.is_shipping = 0  AND b.goods_id = '".$val['goods_id']."'";
                ...
                if (isset($productId) && !empty($productId)) {
                    $total['shipping_fee'] = ($shipping_count == 0 AND $weight_price['free_shipping'] == 1) ?0 :$this->shipping_model->shipping_fee($shipping_id, $weight_price['weight'], $total['goods_price'], $weight_price['number'],$shipping_info['shipping_area_id']);
                } else {
                    $total['shipping_fee'] = ($shipping_count == 0 AND $weight_price['free_shipping'] == 1) ?0 :$this->shipping_model->shipping_fee($shipping_id, $weight_price['weight'], $total['goods_price'], $weight_price['number'],$shipping_info['shipping_area_id']);
                }
            ...
            foreach ($data as $k => $v) {
                ...
                foreach ($goods_data as $a => $d) {
                    if ($d['extension_code'] == "package_buy") {
                        $sql = "select * from  ecs_goods_activity where act_id ='" . $d['goods_id'] . "'";
...
//                }
        ...
        if ($status == 1) {
            $sql = "SELECT order_sn AS order_id,order_amount AS total_amount,user_id AS pay_status,shipping_status ,order_id AS id 
                    FROM ecs_order_info WHERE user_id = {$user_id} AND (order_status = 0 OR order_status = 1) AND pay_status = 0 AND shipping_status = 0  order by id desc limit {$page},{$page_size}";
            $data = $this->getORM()->queryRows($sql);
            foreach ($data as $k => $v) {
                $sql = "select * from  ecs_order_goods WHERE order_id = " . $v['id']. "";
                $order_goods = $this->getORM()->queryRow($sql);
                   if ($order_goods['extension_code'] == "package_buy") {
                       $sql = "SELECT a.goods_name AS name,a.goods_id,a.goods_number AS number,a.goods_price AS price,a.goods_attr AS spec,b.package_image AS list_pic_url
                        FROM ecs_order_goods AS a
                        LEFT JOIN ecs_goods_activity AS b ON a.goods_id = b.act_id
                        WHERE order_id = " . $v['id'] . "";
                } else {
            ...
            $sql = "SELECT COUNT(order_sn) AS num FROM ecs_order_info WHERE user_id = {$user_id} AND (order_status = 0 OR order_status = 1) AND pay_status = 0 AND shipping_status = 0";
            $page_total = $this->getORM()->queryRows($sql);
            $return['pagetotal'] = ceil($page_total[0]['num'] / $page_size);
//            var_dump($return);die;
            return $return;
    ...
    public function seckill_fee($goods_id, $number,$consignee,$shipping_id)
    {
        ...
        if ($shipping_id > 0) {
            ...
            if (!empty($shipping_info)) {
                ...
                $this->shipping_model = new ShippingModel();
                $total['shipping_fee'] = ($shipping_count == 0) ?0 :$this->shipping_model->shipping_fee($shipping_id, $shipping_count_data['goods_weight'], $spike_data["spike_sum"], $number,$shipping_info['shipping_area_id']);
            }
        }
