...
class User extends Api
{
    ...
    public function getRules()
    {
        return array(
            ...
            "couponListApi" => array(
                "user_id" => array("name" => "user_id", "require" => true, "min" => 1, "desc" => "用户的ID"),
                "bonus_id" => array("name" => "bonus_id", "require" => false, "desc" => "红包ID"),
                "page" => array("name" => "page", "require" => true, "min" => 1, "desc" => "页数"),
                'token' => array('name' => 'token', 'require' => true, 'min' => 1, 'max' => 32, 'desc' => 'token'),
            ...
            "getWxmpLogoApi" => array(// "image_url" => array('name' => "image_url", "require" => false, "desc" => "微信小程序临时头像链接"),
            ),
            "checkWxmpLogoApi" => array(
                "user_id" => array("name" => "user_id", "require" => true, "min" => 1, "desc" => "用户ID"),
            ),
            "insertWxmpLogoApi" => array(
                "user_id" => array("name" => "user_id", "require" => true, "min" => 1, "desc" => "用户ID"),
                "image_url" => array('name' => "image_url", "require" => false, "desc" => "微信小程序头像"),
                "nickname" => array('name' => "nickname", "require" => false, "desc" => "微信小程序用户名"),
            ),
            "selectMemberMpLogoApi" => array(
                "user_id" => array("name" => "user_id", "require" => true, "min" => 1, "desc" => "用户ID"),
            )
        );
    }
    ...
    public function memberInfoApi()
    {
        ...
        //图片地址
        $login_image = $this->model->login_image();
        ...
        return array('res' => true, 'info' => $data, "allOrderNum" => $order_num, "rank_name" => $rank_name, "login_image" => $login_image, 'kefu_tel' => $kefu_tel, 'kefu_qq' => $kefu_qq,'my_subordinates' =>$my_subordinates);
    }
    ...
    public function logo()
    {
        ...
        return array("login_image"=>$login_image,"apple_login"=>$apple_login);
    }
    ...
    public function couponListApi()
    {
        ...
        $status = $this->status;
        $data = $this->model->getAviBonusList($user_id, $page, $status, $bonus_id);
...
public function getWxmpLogoApi()
    {
        ...
        if (move_uploaded_file($tmp, $filepath . $imgname . ".png")) {
            ...
            $uploadUrl = $pageURL . '/uploadFile/' . $imgname . ".png";
//            return 'https://api.ecshop.test2.shopex123.com/uploadFile/' . $imgname .".png";
            return $uploadUrl;
        }
        return false;
    }

    public function checkWxmpLogoApi()
    {
        $user_id = $this->user_id;
        $data = $this->model->checkWxmpLogoModel($user_id);
        return $data;
    }

    public function insertWxmpLogoApi()
    {
        $user_id = $this->user_id;
        $image_url = $this->image_url;
        $nickname = $this->nickname;
        $data = $this->model->insertWxmpLogoModel($user_id, $image_url, $nickname);
        return $data;
    }

    public function selectMemberMpLogoApi()
    {
        $user_id = $this->user_id;
        $data = $this->model->selectMemberMpLogoModel($user_id);
        return $data;
    }
