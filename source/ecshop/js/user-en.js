...
function is_registered( username )
{
  ...
  var unlen = username.replace(/[^\x00-\xff]/g, "**").length;
    ...
    if(!myreg.test(username) && num.test(username)){
      document.getElementById('username_notice').innerHTML = '* Cannot be a pure number';
...
function register()
{
  ...
  if(myreg.test(username) && !sms_code){
     msg += '- ' + 'SMS verification code cannot be empty' + '\n';
  }
  if (msg.length > 0)
  {
    alert(msg);
...
function validate_vcode_result(result){
  ...
  if(result.content =='succ'){
    ...
    vcode_tip.innerHTML = '* Verification code error';
...
function time(btn){
  if (wait===0) {
    btn.value = "Obtain verification code";
    ...
    if(wait == 120){
      btn.value ="Successfully sent";
    }else{
      btn.value = wait + "seconds to try";
    }
