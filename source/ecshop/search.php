...
else
{
    ...
    //-生鲜 $default_sort_order_type= $_CFG['sort_order_type'] == '0' ? 'goods_id' : ($_CFG['sort_order_type'] == '1' ? 'shop_price' : 'last_update');
	$default_sort_order_type= $_CFG['sort_order_type'] == '0' ? 'sort_order' : ($_CFG['sort_order_type'] == '1' ? 'shop_price' : 'last_update');
	$sort = (isset($_REQUEST['sort'])  && in_array(trim(strtolower($_REQUEST['sort'])), array('sort_order', 'shop_price', 'last_update', 'click_count')))? trim($_REQUEST['sort'])  : $default_sort_order_type;
   //-生鲜 $sort = (isset($_REQUEST['sort']) && in_array(trim(strtolower($_REQUEST['sort'])), array('goods_id', 'shop_price', 'last_update'))) ? trim($_REQUEST['sort'])  : $default_sort_order_type;
    ...
    /* 查询商品 */
    $sql = "SELECT g.goods_id,g.click_count,g.cat_id, g.goods_name, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ".
            ...
            "WHERE g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 $attr_in ".
                "AND (( 1 " . $categories . $keywords . $brand . $min_price . $max_price . $intro . $outstock . " ) ".$tag_where." ) " .
            "ORDER BY $sort $order";//去除了promote_end_date,original_img,增加了click_count
    ...
    while ($row = $db->FetchRow($res))
    {
		...
		$arr[$row['goods_id']]['click_count']  = $row['click_count'];//-生鲜
        $arr[$row['goods_id']]['pictures']             = get_goods_gallery($row['goods_id']);//-生鲜 商品相册				
		$arr[$row['goods_id']]['buysum']          = ec_buysum($row['goods_id'])+get_cum_sales($row['goods_id']); //-生鲜 已销售	
	    $arr[$row['goods_id']]['evaluation']   = get_evaluation_sum($row['goods_id']);/*-生鲜 评价数===调用get_evaluation_sum*/
       ...
       //-生鲜 $arr[$row['goods_id']]['original_img']  = $row['original_img'];
       //-生鲜 $arr[$row['goods_id']]['gmt_end_time']  = $row['promote_end_date'];
        $arr[$row['goods_id']]['url']           = build_uri('goods', array('gid' => $row['goods_id']), $row['goods_name']);
		$arr[$row['goods_id']]['cat_id']          = $row['cat_id'];  //-生鲜
	   $arr[$row['goods_id']]['cat_name']      = $GLOBALS['db']->getOne("select cat_name from ".$GLOBALS['ecs']->table('category')." where cat_id=".$row['cat_id']);//-生鲜
        ...
        $arr[$row['goods_id']]['save_price'] = price_format($row['market_price'] - $row['shop_price']);
