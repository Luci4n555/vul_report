...
function get_cat_articles($cat_id, $page = 1, $size = 20 ,$requirement='')
{
    ...
    if ($requirement != '')
    {
		...
		$sql = 'SELECT article_id, title, author, add_time, article_pic, open_type,description' . //-生鲜，增加了pic,description
    ...
    if ($res)
    {
        while ($row = $GLOBALS['db']->fetchRow($res))
        {
            ...
            $arr[$article_id]['description']       = $row['description'];//-生鲜
            $arr[$article_id]['article_pic'] = $row['article_pic'];//-生鲜
			
			$arr[$article_id]['add_time1']    =date('Y-m',$row['add_time']);//-生鲜
		    $arr[$article_id]['add_time2']    =date('d',$row['add_time']);//-生鲜
