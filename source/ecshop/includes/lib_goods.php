...
function get_child_tree($tree_id = 0)
{
    ...
    {
        $child_sql = 'SELECT cat_id, cat_name,cate_img,cate_img, parent_id, is_show ' .
                'FROM ' . $GLOBALS['ecs']->table('category') .
                "WHERE parent_id = '$tree_id' AND is_show = 1 ORDER BY sort_order ASC, cat_id ASC  Limit 0,12";//-生鲜 增加了cate_img
        ...
        foreach ($res AS $row)
        {
            if ($row['is_show'])
			   ...
			   $three_arr[$row['cat_id']]['cate_img']  ="images/no_picture.gif";//-生鲜
				
				if ($row['cate_img']){
								    $three_arr[$row['cat_id']]['cate_img']   =$row['cate_img'];
							};//-生鲜
...
function get_recommend_goods($type = '', $cats = '')
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        foreach ($result AS $idx => $row)
        {
			...
			$goods[$idx]['evaluation']   = get_evaluation_sum($row['goods_id']);//-生鲜
...
function get_promote_goods($cats = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name,g.goods_number, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
            ...
            " AND g.is_promote = 1 AND promote_start_date <= '$time' AND promote_end_date >= '$time' ";//-生鲜，增加了goods_number
    ...
    foreach ($result AS $idx => $row)
    {
        ...
        $goods[$idx]['goods_img']    = get_image_path($row['goods_id'], $row['goods_img']);
        $goods[$idx]['url']          = build_uri('goods', array('gid' => $row['goods_id']), $row['goods_name']);
		$goods[$idx]['zk_price']   =   number_format($row['promote_price']/$row['market_price']*10, 1, '.', ' ');  //-生鲜 折扣
  	    $goods[$idx]['buy_num']          = get_cum_sales($row['goods_id']); //-生鲜 已销售
	    $goods[$idx]['sy_num']    = $row['goods_number']-$goods[$idx]['buy_num']; //-生鲜 剩余
        $goods[$idx]['sy_w'] = $goods[$idx]['sy_num']/$row['goods_number']*100;//-生鲜
...
function get_category_recommend_goods($type = '', $cats = '', $brand = 0, $min =0,  $max = 0, $ext='')
{
    ...
    $sql =  'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.click_count, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
                'promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, goods_img, b.brand_name ' .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('brand') . ' AS b ON b.brand_id = g.brand_id ' .
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                    "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
            'WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ' . $brand_where . $price_where . $ext;//-生鲜 增加了click_count
    $num = 0;
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $goods[$idx]['brief']        = $row['goods_brief'];
        $goods[$idx]['brand_name']   = $row['brand_name'];
		$goods[$idx]['click_count']      = $row['click_count'];//-生鲜
        $goods[$idx]['short_name']   = $GLOBALS['_CFG']['goods_name_length'] > 0 ?
                                       sub_str($row['goods_name'], $GLOBALS['_CFG']['goods_name_length']) : $row['goods_name'];
        $goods[$idx]['market_price'] = price_format($row['market_price']);
        $goods[$idx]['shop_price']   = price_format($row['shop_price']);
        ...
        $goods[$idx]['url']          = build_uri('goods', array('gid' => $row['goods_id']), $row['goods_name']);
        $goods[$idx]['short_style_name'] = add_style($goods[$idx]['short_name'], $row['goods_name_style']);		
	    $goods[$idx]['buy_num']          = get_cum_sales($row['goods_id']);//-生鲜
		$goods[$idx]['buysum']          = ec_buysum($row['goods_id'])+get_cum_sales($row['goods_id']); //-生鲜
		$goods[$idx]['evaluation']   = get_evaluation_sum($row['goods_id']);//-生鲜
        $idx++;
    }
...
function get_goods_info($goods_id)
{
    ...
    if (isset($row['goods_id']))
    {
		...
		$row['seller_note'] = $row['seller_note'];//-生鲜
		
	    $row['buysum']          = ec_buysum($row['goods_id'])+get_cum_sales($row['goods_id']); //-生鲜
		$row['evaluation']   = get_evaluation_sum($row['goods_id']);//-生鲜
...
function assign_cat_goods($cat_id, $num = 0, $from = 'web', $order_rule = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.is_new, g.is_best, g.is_hot, g.click_count, g.market_price, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
            ...
            'WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND '.
                'g.is_delete = 0 AND (' . $children . 'OR ' . get_extension_goods($children) . ') ';//-生鲜 增加了is_new is_best is_hot click_count
    ...
    foreach ($res AS $idx => $row)
    {
        ...
        $goods[$idx]['url']          = build_uri('goods', array('gid' => $row['goods_id']), $row['goods_name']);
		$goods[$idx]['evaluation']   = get_evaluation_sum($row['goods_id']);//-生鲜
	    $goods[$idx]['content']   = get_comment_content($row['goods_id']);//-生鲜
		$goods[$idx]['user_name']   = get_comment_user_name($row['goods_id']);//-生鲜
		$goods[$idx]['is_best']          = $row['is_best'];   //-生鲜
		$goods[$idx]['is_new']           = $row['is_new'];  //-生鲜
		$goods[$idx]['is_hot']           = $row['is_hot']; //-生鲜
...
function group_buy_info($group_buy_id, $current_num = 0)
{
	...
	$group_buy['goods_desc'] = $GLOBALS['db']->getOne("select goods_desc from ".$GLOBALS['ecs']->table('goods')." where goods_id=".$group_buy['goods_id']);//-生鲜

	$group_buy['market_price'] = $GLOBALS['db']->getOne("select market_price from ".$GLOBALS['ecs']->table('goods')." where goods_id=".$group_buy['goods_id']);//-生鲜

    $tg=group_buy_stat($group_buy['group_buy_id'],0);//-生鲜
    $group_buy['num']=$tg['total_order'];//-生鲜
	
	 $group_buy['buy_num']    = ec_buysum($group_buy['goods_id']); //-生鲜
	 $group_buy['sy_num']    = $group_buy['restrict_amount']-$group_buy['buy_num']; //-生鲜
	 $group_buy['sy_w'] =$group_buy['sy_num']/$group_buy['restrict_amount']*100;//-生鲜
    /* 统计信息 */
...
{  
    ...
    {  
        ...
        {  
            // 获取当前分类名及其子类  
            $sql = 'SELECT a.cat_id, a.cat_name, a.cat_desc,a.fdlink, a.sort_order AS parent_order, a.cat_id, ' .  
                   'b.cat_id AS child_id, b.cat_name AS child_name, b.cat_desc AS child_desc, b.fdlink AS child_fdlink, b.sort_order AS child_order ' .  
        ...
        {  
            ...
            {  
                //获取当前分类、兄弟及其父类  
                $sql = 'SELECT a.cat_id, a.cat_name, a.cat_desc, a.fdlink, b.cat_id AS child_id, b.cat_name AS child_name, b.cat_desc AS child_desc, b.fdlink AS child_fdlink, b.sort_order ' .  
                       'FROM ' . $GLOBALS['ecs']->table('category') . ' AS a ' .  
                       'LEFT JOIN ' . $GLOBALS['ecs']->table('category') . ' AS b ON b.parent_id = a.cat_id ' .  
        ...
        {  
			...
			$cat_arr[$row['cat_id']]['fdlink'] = $row['fdlink'];
            ...
            if ($row['child_id'] != NULL){  
				...
				$cat_arr[$row['cat_id']]['children'][$row['child_id']]['fdlink'] = $row['child_fdlink'];  
            }  
        }  
    return $cat_arr;  
    }  
...
function get_cat_id_goods_list($cat_id = '', $num = '') {  
    ...
    $sql = 'Select g.goods_id, g.cat_id,c.parent_id, g.goods_name, g.click_count, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, g.is_shipping, ' .  
           "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".  
	...
	$sql .= "ORDER BY g.sort_order DESC LIMIT $num";   
    $res = $GLOBALS['db']->getAll($sql);  
    $goods = array();  
    ...
    {  
        ...
        $goods[$idx]['shop_price'] = price_format($row['shop_price']); 
	    $goods[$idx]['buy_num']          = get_cum_sales($row['goods_id']);
		$goods[$idx]['buysum']          = ec_buysum($row['goods_id'])+get_cum_sales($row['goods_id']); 
	    $goods[$idx]['evaluation']   = get_evaluation_sum($row['goods_id']);
        $goods[$idx]['thumb'] = empty($row['goods_thumb']) ? $GLOBALS['_CFG']['no_picture'] : $row['goods_thumb'];  
        $goods[$idx]['goods_img'] = empty($row['goods_img']) ? $GLOBALS['_CFG']['no_picture'] : $row['goods_img'];  
        $goods[$idx]['url'] = build_uri('goods', array('gid' => $row['goods_id']), $row['goods_name']);  
		$goods[$idx]['click_count']      = $row['click_count'];
	   // $goods[$idx]['sales_count']           = get_sales_count($row['goods_id']);
...
{
    //-生鲜 $sql = "SELECT sum(goods_number) FROM " . $GLOBALS['ecs']->table('order_goods') . " AS g ,".$GLOBALS['ecs']->table('order_info') . " AS o WHERE o.order_id = g.order_id and g.goods_id = " . $goods_id ;
...
{
$sql = "SELECT AVG(comment_rank)*12 as goods_crank FROM " . $GLOBALS['ecs']->table('comment') . " WHERE status=1 and  comment_type =0 and parent_id=0 and id_value =".$goods_id ;
   return $GLOBALS['db']->getOne($sql);
}
...
function get_comment_user_name($goods_id)
{
$sql = "SELECT user_name FROM " . $GLOBALS['ecs']->table('comment') . " WHERE status=1 and  comment_type =0 and parent_id=0 and id_value =" . $goods_id . "  ORDER BY comment_id DESC LIMIT 1";
  return $GLOBALS['db']->getOne($sql);
 
} 
function get_comment_content($goods_id)
{
$sql = "SELECT content FROM " . $GLOBALS['ecs']->table('comment') . " WHERE status=1 and  comment_type =0 and parent_id=0 and id_value =" . $goods_id . "  ORDER BY comment_id DESC LIMIT 1";
  return $GLOBALS['db']->getOne($sql);
