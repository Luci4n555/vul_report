...
{
    $where = " session_id = '" . SESS_ID . "' ";//-生鲜
    if ($_SESSION['user_id'])
    {
        $where = " user_id = '".intval($_SESSION['user_id'])."' ";
    }
    $sql = "SELECT c.rec_id, c.user_id, c.goods_id,g.is_on_sale, c.goods_name, g.goods_thumb, c.goods_sn, c.goods_number, " .
            ...
            " WHERE session_id = '" . SESS_ID . "' " ." or c.user_id=".$_SESSION['user_id'].
            " AND rec_type = '$type'";//生鲜，增加了goods_thumb
    ...
    foreach ($arr as $key => $value)
    {
        ...
        $arr[$key]['goods_thumb']= get_image_path($value['goods_id'], $value['goods_thumb'], true); //生鲜
...
function clear_cart($type = CART_GENERAL_GOODS)
{
    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart') .
            " WHERE session_id = '" . SESS_ID . "' AND rec_type = '$type' OR rec_type = '" . CART_REPURCHASE . "'";
    $GLOBALS['db']->query($sql);
}
...
function get_cart_goods($rec_type = CART_GENERAL_GOODS)
{
    ...
    if ($_SESSION['user_id'] > 0)
    {
        $sql = "SELECT *, IF(parent_id, parent_id, goods_id) AS pid " .
            " FROM " . $GLOBALS['ecs']->table('cart') . " " .
            " WHERE user_id = '". $_SESSION['user_id'] ."' AND rec_type = '" . $rec_type . "'OR rec_type = '" . CART_REPURCHASE . "'" .
    ...
    else
    {
        $sql = "SELECT *, IF(parent_id, parent_id, goods_id) AS pid " .
            " FROM " . $GLOBALS['ecs']->table('cart') . " " .
            " WHERE session_id = '" . SESS_ID . "' AND rec_type = '" . $rec_type . "'OR rec_type = '" . CART_REPURCHASE . "'" .
            " ORDER BY pid, parent_id";
    }
