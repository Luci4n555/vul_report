 ...
 * @return  void
 */
function log_account_change($user_id, $user_money = 0, $frozen_money = 0, $rank_points = 0, $pay_points = 0, $change_desc = '', $change_type = ACT_OTHER,$order_sn=0)
{
    /* 插入帐户变动记录 */
    $account_log = array(
        'user_id'       => $user_id,
 ...
 *
 * @access      public
 * @param       string       $goods        字符串

 *
 * @return      string
 */
function get_goods_attr_one($goods)
{
    $goods_string = '';
    $goods_attr = explode("," , $goods);
    sort($goods_attr);
    foreach($goods_attr as $k =>$v){
        if ($k == 0){
            $goods_string .= $v ;
        }else{
            $goods_string = $goods_string."|".$v;
        }
 ...
 *
 * */
function set_vcode($mobile_phone){
    $vcode = rand(1000,9999);
    $add_time = time();
//    $sql = "delete from ecs_vcode where mobile = '".$mobile_phone."'";
//    $GLOBALS['db']->query($sql);
    $sql = "insert into ecs_vcode (mobile,vcode,add_time) values ('".$mobile_phone."','".$vcode."','".$add_time."')";
...
function create_env($arr,$file='h5')
{
    global $err, $_LANG;
    $replace = array();
...
function format_image($url_img)
{
    $preg = "/http[s]?:\/\/[\w.]+[\w\/]*[\w.]*\??[\w=&\+\%]*/is";

    if (preg_match($preg,$url_img)) {

        return $url_img;

    } else {
     
        return $_SERVER['REQUEST_SCHEME']."://".$_SERVER['SERVER_NAME'].'/'.$url_img;
    }
}
