...
$smarty->assign('type', empty($_GET['type']) ? 0 : $_GET['type']);
...
if ($_REQUEST['step'] == 'add_to_cart') {
    ...
    if (!is_numeric($goods->number) || intval($goods->number) <= 0) {
        ...
        if (addto_cart($goods->goods_id, $goods->number, $goods->spec, $goods->parent)) {
            ...
            //fd30 弹出
            $rows = $GLOBALS['db']->getRow("select goods_brief,shop_price,goods_name,goods_thumb 
from " . $GLOBALS['ecs']->table('goods') . " where goods_id=" . $goods->goods_id);
            ...
            $result['goods_id'] = $goods->goods_id;
            $sql = 'SELECT SUM(goods_number) AS number, SUM(goods_price * goods_number) AS 
amount' .
...
//            $result['goods_number'] = $rowss['number'];//fd30 end弹出
//商品假如购物车数量不对，未登录的数量带入到登录数量中
            $sesso = SESS_ID;
            if (empty($_SESSION['user_name']) || empty($_SESSION['user_id'])) {
                $result['goods_number'] = $rowss['number'];//fd30 end弹出
            } else {
                $sql = "SELECT COUNT(*) AS goods_number FROM " . $GLOBALS['ecs']->table('cart') . " WHERE session_id = '" . SESS_ID . "'";
                //$sql = "SELECT COUNT(*) AS goods_number FROM ".$GLOBALS['ecs']->table('cart')." WHERE user_id = '".$_SESSION['user_id']."'";//-生鲜模板独有  
                $sum = $GLOBALS['db']->getRow($sql);
                $result['goods_number'] = $sum['goods_number'];//fd30 end弹出
            }
        } else {
...
elseif ($_REQUEST['step'] == 'link_buy') {
    ...
    if ($_SESSION['user_id']) {
        $where = "session_id = '" . SESS_ID . "'";
       //-生鲜模板独有 $where = "user_id = '".intval($_SESSION['user_id'])."'";
    }
...
//        $where = "user_id = '" . intval($_SESSION['user_id']) . "'";
    ...
    /* 检查购物车中是否有商品 */
    $sql = "SELECT COUNT(*) FROM " . $ecs->table('cart') .
        " WHERE " . $where . " AND parent_id = 0 AND is_gift = 0 AND rec_type = '$flow_type' OR rec_type = 5";
...
elseif ($_REQUEST['step'] == 'done') {
    ...
    $sql = "SELECT COUNT(*) FROM " . $ecs->table('cart') .
        " WHERE " . $where .
        "AND parent_id = 0 AND is_gift = 0 AND rec_type = '$flow_type' OR rec_type = '" . CART_REPURCHASE . "'";
    if ($db->getOne($sql) == 0) {
        show_message($_LANG['no_goods_in_cart'], '', '', 'warning');
    }
    ...
    foreach ($cart_goods as $val) {
        ...
        if (!$val['is_on_sale'])
        {
            show_message($val['goods_name'].'：该商品已下架！', '返回购物车', 'flow.php', 'warning');
        } 
        /* 统计实体商品的个数 */
        if ($val['is_real']) {
            $is_real_good = 1;
    ...
    $sql = "INSERT INTO " . $ecs->table('order_goods') . "( " .
        ...
        "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id" .
        " FROM " . $ecs->table('cart') .
        " WHERE " . $where . " AND rec_type = '$flow_type' OR rec_type = '" . CART_REPURCHASE . "'";
    ...
    /* 取得支付信息，生成支付代码 */
    if ($order['order_amount'] > 0) {
        if ($payment['pay_code'] == 'paypal') {
//            require __DIR__.'payment/payment.php';
            ecs_header('Location: payment/payment.php?name=abc');
            die;
        }
        $payment = payment_info($order['pay_id']);
    ...
    $smarty->assign('order', $order);
    $smarty->assign('total', $total);
    $smarty->assign('amount_formated', $total['amount_formated']);
    $smarty->assign('goods_list', $cart_goods);
...
} elseif ($_REQUEST['step'] == 'get_erp') {
    $order = $_GET['order_sn'];
    include_once(ROOT_PATH . 'includes/cls_matrix.php');
    $matrix = new matrix;
    $matrix->createOrder($order, 'ecos.ome');
    user_uc_call('add_feed', array($order, BUY_GOODS));

} elseif ($_REQUEST['step'] == 'get_taoda') {
    $order = $_GET['order_sn'];
    include_once("includes/cls_matrix.php");
    $matrix = new matrix();
    $bind_info = $matrix->get_bind_info(array('taodali'));
    if ($bind_info) {
        $matrix->createOrder($order, 'taodali');
    }
} elseif ($_REQUEST['step'] == 'pay_status') {
    $order = $_GET['order_sn'];
    include_once("includes/cls_matrix.php");
    $matrix = new matrix();
    // 请求erp
    ...
    if ($bind_info) {
        $matrix->updateOrder($order, 'ecos.ome');
    }
}
/*------------------------------------------------------ */
//-- 更新购物车
