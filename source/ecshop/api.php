...
function update_order_status(){
    ...
    $data = $_POST;
    $order_sn = str_replace($vowels,'',trim($data['order_id']));
    $status = str_replace($vowels,'',trim($data['status']));
    $type =  str_replace($vowels,'',trim($data['type']));
    $lastmodify = gmtime();
...
function ome_create_payments(){
    ...
    $sql = "select pay_id,pay_name from " . $GLOBALS['ecs']->table('payment') . " where pay_code = '".$data['pay_type']."'  ";
