...
$default_sort_order_type   = $_CFG['sort_order_type'] == '0' ? 'sort_order' : ($_CFG['sort_order_type'] == '1' ? 'shop_price' : 'last_update');// goods_id 修改为 sort_order -生鲜

$sort  = (isset($_REQUEST['sort'])  && in_array(trim(strtolower($_REQUEST['sort'])), array('sort_order', 'shop_price', 'last_update', 'click_count'))) ? trim($_REQUEST['sort'])  : $default_sort_order_type;// goods_id 修改为 sort_order 增加了click_count -生鲜
...
function brand_recommend_goods($type, $brand, $cat = 0)
{
    ...
    foreach ($result AS $row)
    {
            ...
            $row['promote_start_date'] <= $time && $row['promote_end_date'] >= $time))
        {
			...
			$goods[$idx]['buysum']          = ec_buysum($row['goods_id'])+get_cum_sales($row['goods_id']); // 已销售数量-生鲜
	        $goods[$idx]['evaluation']   = get_evaluation_sum($row['goods_id']);//评价数===调用get_evaluation_sum 生鲜
...
//增加click_count调用 -生鲜
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$arr[$row['goods_id']]['buysum']          = ec_buysum($row['goods_id'])+get_cum_sales($row['goods_id']); //已销售-生鲜	
        $arr[$row['goods_id']]['save_price'] = price_format($row['market_price'] - $row['shop_price']);
	    $arr[$row['goods_id']]['evaluation']   = get_evaluation_sum($row['goods_id']);//评价数===调用get_evaluation_sum -生鲜
