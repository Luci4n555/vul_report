        ...
        if(preg_match("/^1[3456789]{1}\d{9}$/",$username)){
            if($sms_code != '' && $sms_code == $_SESSION[$username.'-sms_code']){
                ...
                if (time() - $_SESSION[$username.'-send_time'] > 600) {
                    show_message($_LANG['sms_veri_expir'], $_LANG['back_up_page'], 'user.php?act=register', 'error');
                }
                $other['mobile_phone'] = $username;
                $_SESSION[$username.'-sms_code'] ='false';
            }else{
                show_message($_LANG['sms_veri_err'], $_LANG['sign_up'], 'user.php?act=register', 'error');
            }
        }
...
elseif ($action == 'act_add_message')
{
    include_once(ROOT_PATH . 'includes/lib_clips.php');
        ...
        // 验证文件类型为PDF时判断
    if ($_FILES['message_img']['type'] === 'application/pdf') {
        detectXSSInjection($_FILES['message_img']['tmp_name']);

        // 检测上传的 PDF 文件
        if ($_FILES['message_img']['error'] === UPLOAD_ERR_OK) {
            $tmp_name = $_FILES['message_img']['tmp_name'];

            if (detectXSSInjection($tmp_name)) {
                die($_LANG['message_pdf']);
            }
        }
    }
...
//    $ip = $_SERVER["REMOTE_ADDR"];
    ...
    if ($_POST['sms_code'] != '' && $_POST['username'] != '')
    {
        ...
        if($sms_code == $_SESSION[$username.'-sms_code']){
            // 短信验证码十分钟内有效
            if (time() - $_SESSION[$username.'-send_time'] > 600) {
                show_message($_LANG['sms_veri_expir'], $_LANG['back_up_page'], 'user.php?act=sms_get_password', 'info');
            }
            ...
            show_message($_LANG['sms_veri_err'], $_LANG['back_up_page'], 'user.php?act=sms_get_password', 'info');
        }
    }
    else
    {
...
// 检测 PDF 文件是否携带 XSS 注入
function detectXSSInjection($file)
{
    // 读取 PDF 文件的内容
    ...
    // 检测文本内容中是否包含 <script> 标签等潜在的 XSS 注入代码
    if (preg_match('/<script\b[^>]*>(.*?)<\/script>/is', $text)) {
        die($_LANG['message_pdf_xss']);
    }

    return true; // 未检测到 XSS 注入
}