...
//    }
    ...
    if (!$smarty->is_cached('group_buy_goods.dwt', $cache_id))
    {
        ...
        $smarty->assign('top_goods',  get_top10());           // 销售排行
        $smarty->assign('promotion_info', get_promotion_info());
		$smarty->assign('pictures',            get_goods_gallery($goods_id));      //-生鲜 商品相册
...
elseif ($_REQUEST['act'] == 'buy')
{
    ...
    /* 更新：加入购物车 */
    //-生鲜 $goods_price = $group_buy['deposit'] > 0 ? $group_buy['deposit'] : $group_buy['cur_price'];
    $goods_price = $group_buy['cur_price'];
    $cart = array(
...
/*    $sql = "SELECT b.*, IFNULL(g.goods_thumb, '') AS goods_thumb, b.act_id AS group_buy_id, ".
            ...
            "FROM " . $GLOBALS['ecs']->table('goods_activity') . " AS b " .
                "LEFT JOIN " . $GLOBALS['ecs']->table('goods') . " AS g ON b.goods_id = g.goods_id " .
            "WHERE b.act_type = '" . GAT_GROUP_BUY . "' " .
            " AND b.is_finished < 3 ORDER BY b.act_id DESC";*/
    ...
    while ($group_buy = $GLOBALS['db']->fetchRow($res))
    {
        ...
        //$group_buy['formated_start_date']   = local_date($GLOBALS['_CFG']['time_format'], $group_buy['start_date']);
		  $group_buy['formated_start_date']   = local_date('M d, Y H:i:s',$group_buy['start_date']);//-生鲜
        //$group_buy['formated_end_date']     = local_date($GLOBALS['_CFG']['time_format'], $group_buy['end_date']);
		  $group_buy['formated_end_date']     = local_date('M d, Y H:i:s',$group_buy['end_date']);//-生鲜
	    ...
	    /*-生鲜 显示市场价*/
	    $group_buy['market_price'] = $GLOBALS['db']->getOne("select market_price from ".$GLOBALS['ecs']->table('goods')." where goods_id=".$group_buy['goods_id']);
		
	    $group_buy['goods_brief'] = $GLOBALS['db']->getOne("select goods_brief from ".$GLOBALS['ecs']->table('goods')." where goods_id=".$group_buy['goods_id']);
		
		 $group_buy['buy_num']    = ec_buysum($group_buy['goods_id']); //-生鲜 已销售
		 $group_buy['sy_num']    = $group_buy['restrict_amount']-$group_buy['buy_num']; //-生鲜 剩余
         $group_buy['sy_w'] =$group_buy['sy_num']/$group_buy['restrict_amount']*100;
