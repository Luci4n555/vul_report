...
if ($_REQUEST['act'] == 'list')
{
    ...
    foreach ($log_list['list'] as $key => $value) {
        $log_list['list'][$key]['x_ip_address'] = substr_cut($log_list['list'][$key]['ip_address']);
    }
...
function get_admin_logs()
{
    ...
    $filter = array();
    $filter['sort_by'] = empty($_REQUEST['sort_by']) ? 'al.log_id' : mysqli_real_escape_string($db->link_id, trim($_REQUEST['sort_by']));
