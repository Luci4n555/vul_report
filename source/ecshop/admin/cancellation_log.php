...
//    admin_priv('surplus_manage');
    ...
    $smarty->assign('ur_here', $_LANG['12_user_cancellation_log']);
...
//    $smarty->assign('action_link', array('text' => $_LANG['surplus_add'], 'href' => 'user_account.php?act=add'));
    ...
    assign_query_info();
    $smarty->display('user_cancellation_log.htm');
}
///*------------------------------------------------------ */
////-- 通过
///*------------------------------------------------------ */
...
elseif ($_REQUEST['act'] == 'query') {
    ...
    make_json_result($smarty->fetch('user_cancellation_log.htm'), '', array('filter' => $list['filter'], 'page_count' => $list['page_count']));
}
/*------------------------------------------------------ */
//-- ajax删除一条信息
/*------------------------------------------------------ */
...
function account_list()
{
    ...
    if ($result === false) {
        ...
        /* 查询数据 */
        $sql = 'SELECT * FROM ' .
            $GLOBALS['ecs']->table('cancellation_log') .
