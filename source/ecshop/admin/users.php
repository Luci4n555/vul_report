...
if ($_REQUEST['act'] == 'list')
{
    ...
    foreach ($user_list['user_list'] as $key => $value) {
        $user_list['user_list'][$key]['x_user_name'] = substr_cut($user_list['user_list'][$key]['user_name']);
        $user_list['user_list'][$key]['x_email'] = substr_cut($user_list['user_list'][$key]['email']);
    }
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $username = filter_compile($_POST['username']);
    $password = empty($_POST['password']) ? '' : trim($_POST['password']);
    $email = filter_compile($_POST['email']);
    ...
    $other['recode']        = rand_code();
    $other['msn'] = isset($_POST['extend_field1']) ? filter_compile(trim($_POST['extend_field1'])) : '';
    $other['qq'] = isset($_POST['extend_field2']) ? filter_compile(trim($_POST['extend_field2'])) : '';
    $other['office_phone'] = isset($_POST['extend_field3']) ? filter_compile(trim($_POST['extend_field3'])) : '';
    $other['home_phone'] = isset($_POST['extend_field4']) ? filter_compile(trim($_POST['extend_field4'])) : '';
    $other['mobile_phone'] = isset($_POST['extend_field5']) ? filter_compile(trim($_POST['extend_field5'])) : '';
    ...
    sys_msg(sprintf($_LANG['add_success'], filter_compile(stripslashes($_POST['username']))), 0, $link);
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $row['user_name'] = addslashes($user_name);
    $users  = init_users();
    ...
    if ($row)
    {
        ...
        $user['parent_username']= urldecode($row['parent_username']);
        $user['qq']             = $row['qq'];
        $user['msn']            = $row['msn'];
...
elseif ($_REQUEST['act'] == 'update')
{
    /* 检查权限 */
    admin_priv('users_manage');
    $username = empty($_POST['username']) ? '' : filter_compile($_POST['username']);
    $password = empty($_POST['password']) ? '' : trim($_POST['password']);
    $email = empty($_POST['email']) ? '' : filter_compile($_POST['email']);
    $sex = empty($_POST['sex']) ? 0 : intval($_POST['sex']);
    $sex = in_array($sex, array(0, 1, 2)) ? $sex : 0;
    $birthday = $_POST['birthdayYear'] . '-' .  $_POST['birthdayMonth'] . '-' . $_POST['birthdayDay'];
    $rank = empty($_POST['user_rank']) ? 0 : intval($_POST['user_rank']);
...
function user_list()
{
    ...
    if ($result === false)
    {
        ...
        $vowels = array("<", ">", "?","\"","(","=","\'","-","+","%","!","#","*","`","[","]","{","}","^","|",")",",","?",";"," ");
        $filter['sort_by']    = empty($_REQUEST['sort_by'])    ? 'user_id' : str_replace($vowels,'',trim($_REQUEST['sort_by']));
        $filter['sort_order'] = empty($_REQUEST['sort_order']) ? 'DESC'     : str_replace($vowels,'',trim($_REQUEST['sort_order']));
        ...
        $sql = "SELECT user_id, user_name, email,recode, is_validated, user_money, frozen_money, rank_points, pay_points, reg_time ".
    ...
    for ($i=0; $i<$count; $i++)
    {
        ...
        $user_list[$i]['x_user_name'] = substr_cut($user_list[$i]['user_name']);
        $user_list[$i]['x_email'] = substr_cut($user_list[$i]['email']);
        $sql = "select parent_id from ecs_users where  user_id  = ".$user_list[$i]['user_id'];
                $father_id = $GLOBALS['db']->getRow($sql);
                if($father_id['parent_id']>0){
                    $sql = "select user_name from ecs_users where user_id = ".$father_id['parent_id'];
                    $father = $GLOBALS['db']->getRow($sql);

                    $user_list[$i]['subordinate']= urldecode($father['user_name']);

                }else{
                    $user_list[$i]['subordinate'] = "暂无上级";
                }
    }
