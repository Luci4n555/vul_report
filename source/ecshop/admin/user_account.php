...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    $user_name    = !empty($_POST['user_id'])      ? filter_compile($_POST['user_id'])          : '';
    $admin_note   = !empty($_POST['admin_note'])   ? filter_compile($_POST['admin_note'])       : '';
    $user_note    = !empty($_POST['user_note'])    ? filter_compile($_POST['user_note'])        : '';
    $payment      = !empty($_POST['payment'])      ? filter_compile($_POST['payment'])          : '';
    $bank_addr    = !empty($_POST['bank_addr'])    ? filter_compile($_POST['bank_addr'])        : '';
    $bank_account    = !empty($_POST['bank_account'])    ? filter_compile($_POST['bank_account'])        : '';
    $back_cart =!empty($_POST['back_cart'])    ?filter_compile($_POST['back_cart'])   : '';
    ...
    if ($_REQUEST['act'] == 'insert')
    {
        ...
        $sql = "INSERT INTO " .$ecs->table('user_account').
               " VALUES ('', '$user_id', '$_SESSION[admin_name]', '$amount', '".gmtime()."', '".gmtime()."', '$admin_note', '$user_note', '$process_type', '$payment', '$is_paid','$bank_addr','$bank_account','$back_cart')";
        $db->query($sql);
        $id = $db->insert_id();
    }
    else
...
elseif ($_REQUEST['act'] == 'action')
{
    ...
    if ($account['is_paid'] == 0)
    {
        ...
        if ($is_paid == '1' && $account['process_type'] == '1')
        {
            ...
            //更新会员余额数量
           log_account_change($account['user_id'], $balance, 0, 0, 0, $_LANG['surplus_type_1'], ACT_DRAWING);
        }
...
function account_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('user_account'). " AS ua" .$where;
//        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('user_account'). " AS ua, ".
//                   $GLOBALS['ecs']->table('users') . " AS u " . $where;
