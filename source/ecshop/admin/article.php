...
if ($_REQUEST['act'] == 'insert')
{
    /* 权限判断 */
    admin_priv('article_manage');
    $title = filter_compile($_POST['title']);
    $author = filter_compile($_POST['author']);
    $author_email = filter_compile($_POST['author_email']);
    $keywords = filter_compile($_POST['keywords']);
    $description = filter_compile($_POST['description']);
    /*检查是否重复*/
    $is_only = $exc->is_only('title', $title,0, " cat_id ='$_POST[article_cat]'");
    ...
    $sql = "INSERT INTO ".$ecs->table('article')."(title, cat_id, article_type, is_open, author, ".
            ...
            "VALUES ('$title', '$_POST[article_cat]', '$_POST[article_type]', '$_POST[is_open]', ".
                "'$author', '$author_email', '$keywords', '$_POST[FCKeditor1]', ".
                "'$add_time', '$file_url', '$open_type', '$_POST[link_url]', '$description')";
    $db->query($sql);
...
if ($_REQUEST['act'] =='update')
{
    ...
    $description = filter_compile($_POST['description']);
    /*检查文章名是否相同*/
    $is_only = $exc->is_only('title', $title, $_POST['id'], "cat_id = '$_POST[article_cat]'");
    ...
    /* add专题文章图片 */
    $recommend_url = '';
    if (empty($_FILES['recommend_file']['error']) || (!isset($_FILES['recommend_file']['error']) && isset($_FILES['recommend_file']['tmp_name']) && $_FILES['recommend_file']['tmp_name'] != 'none'))
    {
        // 检查文件格式
        if (!check_file_type($_FILES['recommend_file']['tmp_name'], $_FILES['recommend_file']['name'], $allow_file_types))
        {
            sys_msg($_LANG['invalid_file']);
        }

        // 复制文件
        $res = upload_article_file($_FILES['recommend_file']);
        if ($res != false)
        {
            $recommend_url = $res;
        }
    ...
    if ($recommend_url == '')
    {
        $recommend_url = compile_str($_POST['article_pic']);
    }


    /* 如果 file_url 跟以前不一样，且原来的文件是本地文件，删除原来的文件 */
    $sql = "SELECT article_pic FROM " . $ecs->table('article') . " WHERE article_id = '$_POST[id]'";
    $old_url = $db->getOne($sql);
    if ($old_url != '' && $old_url != $recommend_url && strpos($old_url, 'http://') === false && strpos($old_url, 'https://') === false)
    {
    ...
    if ($exc->edit("title='$title', cat_id='$_POST[article_cat]', article_type='$_POST[article_type]', is_open='$_POST[is_open]', author='$author', author_email='$author_email', keywords ='$keywords', file_url ='$file_url',article_pic='$recommend_url', open_type='$open_type',is_recommend='$_POST[is_recommend]', content='$_POST[FCKeditor1]', link='$_POST[link_url]', description = '$description', is_index = '$_POST[is_index]'", $_POST['id']))
    {
...
function get_articleslist()
{
    ...
    if ($result === false)
    {
        ...
        $filter['sort_by'] = empty($_REQUEST['sort_by']) ? 'a.article_id' : mysqli_real_escape_string($db->link_id, trim($_REQUEST['sort_by']));
