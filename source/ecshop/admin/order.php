...
elseif ($_REQUEST['act'] == 'list')
{
    ...
    foreach ($order_list['orders'] as $key => $value) {
        $order_list['orders'][$key]['x_address'] = substr_cut($order_list['orders'][$key]['address']);
        $order_list['orders'][$key]['x_buyer'] = substr_cut($order_list['orders'][$key]['buyer']);
        $order_list['orders'][$key]['x_consignee'] = substr_cut($order_list['orders'][$key]['consignee']);
        $order_list['orders'][$key]['x_tel'] = substr_cut($order_list['orders'][$key]['tel']);
    }
...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    $order['x_consignee'] = substr_cut($order['consignee']);
    $order['x_mobile'] = substr_cut($order['mobile']);
    $order['x_address'] = substr_cut($order['address']);
    $order['x_user_name'] = substr_cut($order['user_name']);
    $order['x_email'] = substr_cut($order['email']);
    $order['x_tel'] = substr_cut($order['tel']);
    ...
    if ($order['user_id'] > 0)
    {
        ...
        $user['x_email'] = substr_cut($user['email']);
        ...
        foreach ($address_list as $key => $value) {
            $address_list[$key]['x_consignee'] = substr_cut($address_list[$key]['consignee']);
            $address_list[$key]['x_email'] = substr_cut($address_list[$key]['email']);
            $address_list[$key]['x_address'] = substr_cut($address_list[$key]['address']);
            $address_list[$key]['x_tel'] = substr_cut($address_list[$key]['tel']);
            $address_list[$key]['x_mobile'] = substr_cut($address_list[$key]['mobile']);
        }
...
elseif ($_REQUEST['act'] == 'delivery_list')
{
    ...
    foreach ($result['delivery'] as $key => $value) {
        $result['delivery'][$key]['x_consignee'] = substr_cut($result['delivery'][$key]['consignee']);
    }
...
elseif ($_REQUEST['act'] == 'delivery_info')
{
    ...
    $delivery_order['x_user_name'] = substr_cut($delivery_order['user_name']);
    $delivery_order['x_consignee'] = substr_cut($delivery_order['consignee']);
    $delivery_order['x_tel'] = substr_cut($delivery_order['tel']);
    $delivery_order['x_mobile'] = substr_cut($delivery_order['mobile']);
    $delivery_order['x_email'] = substr_cut($delivery_order['email']);
    $delivery_order['x_address'] = substr_cut($delivery_order['address']);
...
elseif ($_REQUEST['act'] == 'delivery_ship')
{
    ...
    $delivery['invoice_no'] = isset($_REQUEST['invoice_no']) ? filter_compile($_REQUEST['invoice_no']) : '';
    $action_note    = isset($_REQUEST['action_note']) ? filter_compile($_REQUEST['action_note']) : '';
    ...
    $sql = "SELECT DG.goods_id, DG.is_real, DG.product_id, DG.send_number AS sums, IF(DG.product_id > 0, P.product_number, G.goods_number) AS storage, G.goods_name, DG.send_number
        FROM " . $GLOBALS['ecs']->table('delivery_goods') . " AS DG, " . $GLOBALS['ecs']->table('goods') . " AS G, " . $GLOBALS['ecs']->table('products') . " AS P
        WHERE DG.goods_id = G.goods_id
        AND DG.delivery_id = '$delivery_id'
        AND DG.product_id = 0
        GROUP BY DG.rec_id ";
    $delivery_result = $GLOBALS['db']->getAll($sql);
    ...
    if(!empty($delivery_stock_result))
    {
        ...
        foreach ($delivery_result as $k => $v){
            if ($v['product_id'] == 0){
                $delivery_stock_result[] = $v;
            }
        }
    }
    else
    ...
    //发货通知APP
    //$is_push = delivery_msg_push($delivery_id,$db,$ecs);//取消推送
    /* 清除缓存 */
    clear_cache_files();
...
elseif ($_REQUEST['act'] == 'back_list')
{
    ...
    foreach ($result['back'] as $key => $value) {
        $result['back'][$key]['x_consignee'] = substr_cut($result['back'][$key]['consignee']);
    }
...
elseif ($_REQUEST['act'] == 'back_info')
{
    ...
    $back_order['x_consignee'] = substr_cut($back_order['consignee']);
    $back_order['x_email'] = substr_cut($back_order['email']);
    $back_order['x_tel'] = substr_cut($back_order['tel']);
    $back_order['x_mobile'] = substr_cut($back_order['mobile']);
    $back_order['x_address'] = substr_cut($back_order['address']);
    $back_order['x_user_name'] = substr_cut($back_order['user_name']);
        ...
        if (isset($_POST['rec_id']))
        {
            foreach ($_POST['rec_id'] AS $key => $rec_id)
            {
                $sql = "SELECT goods_number ".
                    ...
                    "WHERE goods_id =".filter_compile($_POST['goods_id'][$key]);
                /* 取得参数 */
                ...
                if($product_id)
                {
                    ...
                    $sql = "SELECT product_number ".
                        ...
                        " WHERE product_id =".filter_compile($_POST['product_id'][$key]);
                }
    ...
    elseif ('consignee' == $step)
    {
        ...
        elseif (isset($_POST['finish']))
        {
            if (strlen($_POST['mobile']) != 11 && !empty($_POST['mobile'])){
                $links[] = array('text' => '取消等待，立即返回', 'href' => 'order.php?act=list');
                sys_msg("手机号码不合法", 0, $links);
            }
            if (strlen($_POST['tel']) != 11 && !empty($_POST['tel'])){
                $links[] = array('text' => '取消等待，立即返回', 'href' => 'order.php?act=list');
                sys_msg("电话不合法", 0, $links);
            }
...
{
    /* 检查权限 */
    admin_priv('return_list');
    /* 查询 */
    $result = return_list();
    foreach ($result['return_list'] as $k =>$v ){
        $sql ="select * from ecs_users where user_id ='".$v['user_id']."'";
        $user_data = $GLOBALS['db']->getRow($sql);
        $result['return_list'][$k]['user_name'] = utf8_decode($user_data['user_name']);
        $sql ="select * from ecs_goods where goods_id ='".$v['goods_id']."'";
        $goods_data = $GLOBALS['db']->getRow($sql);
        $result['return_list'][$k]['goods_name'] = $goods_data['goods_name'];

    }
    /* 模板赋值 */
    $smarty->assign('ur_here', $_LANG['11_return_order']);
    ...
    $smarty->assign('return_list',   $result['return_list']);
    $smarty->assign('filter',       $result['filter']);
    $smarty->assign('record_count', $result['record_count']);
    $smarty->assign('page_count',   $result['page_count']);
    $smarty->assign('sort_update_time', '<img src="images/sort_desc.png">');
    ...
    /* 显示模板 */
    assign_query_info();
    $smarty->display('return_list.htm');
}
elseif ($_REQUEST['act'] == 'return_succ'){
    $sql ="update  ecs_return_goods set return_status ='succ' where r_id='".$_GET['r_id']."'";
    $GLOBALS['db']->query($sql);

    //查询当前订单的金额
    $sql = "select a.goods_amount,a.user_id from ecs_order_info as a left join ecs_return_goods as b on a.order_id = b.order_id where b.r_id = '".$_GET['r_id']."'";
    $defult = $GLOBALS['db']->getRow($sql);
    $defult_user_id = $defult['user_id'];
    $this_goods_amount = $defult['goods_amount'];//订单的金额

    //查询当前订单的用户余额
    $mysql = "select * from ecs_users where user_id = '$defult_user_id'";
    $user_money = $GLOBALS['db']->getRow($mysql);
    $user_money_num = $user_money['user_money'];//当前这个订单用户的余额

    //修改当前用户的余额
    $money = $user_money_num + $this_goods_amount;//计算金额
    $save = "update ecs_users set user_money = '$money' where user_id = '$defult_user_id'";
    $save_result = $GLOBALS['db']->getRow($save);

    //将退款记录写入数据表中
    $time = time();
    $insert = "insert into ecs_account_log(user_id,user_money,frozen_money,rank_points,pay_points,change_time,change_desc,change_type) values ('$defult_user_id','$this_goods_amount','0.00','0','0','$time','订单退款：$this_goods_amount','44')";
    $GLOBALS['db']->query($insert);


    $sql = "select * from ecs_return_goods where r_id ='".$_GET['r_id']."'";
    $data =  $GLOBALS['db']->getRow($sql);

    $sql = "UPDATE ecs_order_info SET shipping_status = '2' WHERE user_id = '".$data['user_id']."' AND order_sn ='".$data['order_sn']."' ";
    $GLOBALS['db']->query($sql);

    // 将次操作记录到订单信息中   默认 已发货 已支付
    $sql = "insert into ecs_order_action set order_id = '".$data['order_id']."',action_user = 'admin',order_status = '4',shipping_status = '1',pay_status='2',action_note='同意当前订单售后的申请',log_time='".time()."'";

    $GLOBALS['db']->query($sql); // 将此次操作记录到订单信息中

    $lnk[] = array('text' =>'返回首页',    'href'=>'order.php?act=return_list');
    sys_msg('操作成功', 0, $lnk);

}
elseif ($_REQUEST['act'] == 'return_save'){
    $r_id = $_GET['r_id'];
    $smarty->assign('r_id', $r_id);
    $smarty->display('return_save.html');

}
elseif ($_REQUEST['act'] == 'return_gun'){
    $sql ="update  ecs_return_goods set return_status ='error' where r_id='".$_POST['r_id']."'";
    $GLOBALS['db']->query($sql);
    // 将次操作记录到订单信息中  默认 已发货 已支付
    $sql = "insert into ecs_order_action set order_id = '".$data['order_id']."',action_user = 'admin',order_status = '4',shipping_status = '1',pay_status='2',action_note='拒绝了当前订单的售后申请',log_time='".time()."'";
    $GLOBALS['db']->query($sql); // 将此次操作记录到订单信息中
    $lnk[] = array('text' =>'返回首页',    'href'=>'order.php?act=return_list');
    sys_msg('操作成功', 0, $lnk);
}
...
elseif ($_REQUEST['act'] == 'batch_operate_post')
{
    ...
    $action_note= filter_compile($_REQUEST['action_note']);     // 操作备注
...
elseif ($_REQUEST['act'] == 'operate_post')
{
    ...
    /* 取得备注信息 */
    $action_note = filter_compile($_REQUEST['action_note']);
...
function return_list()
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('return_goods') . $where;
    ...
    {
        $row[$key]['ctime'] = local_date($GLOBALS['_CFG']['date_format'],$value['ctime']);

    }
    $arr = array('return_list' => $row, 'filter' => $filter, 'page_count' => $filter['page_count'], 'record_count' => $filter['record_count']);
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        $filter['platform'] = empty($_REQUEST['platform']) ? '' : trim($_REQUEST['platform']);
     //   $filter['sort_by'] = empty($_REQUEST['sort_by']) ? 'add_time' : trim($_REQUEST['sort_by']);
        $filter['sort_order'] = empty($_REQUEST['sort_order']) ? 'DESC' : trim($_REQUEST['sort_order']);
        $filter['sort_by'] = empty($_REQUEST['sort_by']) ? 'add_time' : mysqli_real_escape_string($db->link_id, trim($_REQUEST['sort_by']));
        if (strpos($filter['sort_by'], 'updatexml') !== false) return false;
        ...
        /* 查询 */
        $sql = "SELECT o.order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status,o.platform,o.order_amount, o.money_paid, o.callback_status," .
    ...
    foreach ($row AS $key => $value)
    {
        ...
        $row[$key]['buyer'] = urldecode($value['buyer']);
