...
switch ($action)
{
    ...
    case 'sms_send_vcode':
        admin_priv('sms_send_vcode');
        $smarty->assign('ur_here',      $_LANG['07_sms_send_vcode']);

        $data = sms_list();

        foreach ($data['list'] as $key => $value) {
            $data['list'][$key]['x_mobile'] = substr_cut($data['list'][$key]['mobile']);
        }

        $smarty->assign('data',    $data['list']);
        ...
        assign_query_info();
        $smarty->display('sms_send_vcode.htm');

    break;
    case 'sms_set_vcode':
        admin_priv('sms_set_vcode');
        $sql = "select value from  ecs_shop_config  WHERE code = 'sms_set_update'";
        $res = $GLOBALS['db']->getRow($sql);
        $result = unserialize($res['value']);//数据转换
        $smarty->assign('ur_here',      $_LANG['08_sms_send_vcode']);
        $data = sms_list();
        $smarty->assign('data',    $result);
        $smarty->assign('full_page',    1);
        $smarty->display('sms_set_vcode.html');

        break;
    case 'sms_set_update':
        $accessKeyId = filter_compile($_POST['accessKeyId']);  //赋值
        $accessKeySecret = filter_compile($_POST['accessKeySecret']);
        $SignName = filter_compile($_POST['SignName']);
        $TemplateCode = filter_compile($_POST['TemplateCode']);


        $status = filter_compile($_POST['status']);

        $arr = array("accessKeyId" => $accessKeyId,"accessKeySecret" => $accessKeySecret,"SignName" => $SignName,'TemplateCode'=>$TemplateCode,'status'=>$status);

        $result = serialize($arr);//数据转换
        $sql = "update ecs_shop_config SET value='$result' WHERE code = 'sms_set_update'";
        $res = $GLOBALS['db']->query($sql);
        $smarty->assign('ur_here',      $_LANG['08_sms_send_vcode']);
        $smarty->assign('data',    $data['list']);
        $smarty->assign('full_page',    1);
        $links[] = array('text' => '取消等待，立即返回', 'href' => 'sms.php?act=sms_set_vcode');
        sys_msg("设置成功", 0, $links);//sys_msg()是一个消息提示,自动跳转的方法
        $smarty->display('sms_set_vcode.html');
        break;
    ...
    case 'query':
        ...
        $smarty->assign('sort_user_id', '<img src="images/sort_desc.png">');
        assign_query_info();
        make_json_result($smarty->fetch('sms_send_vcode.htm'), '', array('filter' => $data['filter'], 'page_count' => $data['page_count']));
...
function sms_list()
{
    ...
    /* 分页大小 */
    $filter['record_count'] = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('vcode'));

    $filter = page_and_size($filter);
    $sql = "SELECT * ".
            " FROM " . $GLOBALS['ecs']->table('vcode') . $ex_where .
    ...
    foreach($data as $key=>$item){
        $data[$key]['add_time'] = date('Y-m-d H:i:s',$item['add_time']);
    }


    $arr = array('list' => $data, 'filter' => $filter,
