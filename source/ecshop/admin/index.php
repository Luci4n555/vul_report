...
elseif ($_REQUEST['act'] == 'menu')
{
    ...
    arsort($menus["18_lead_manage"]["children"]);
...
elseif ($_REQUEST['act'] == 'main')
{
    ...
    foreach($version_all as $v){
        ...
        if ( $item['date'] > intval(RELEASE) && $item['version'] == VERSION){
            $update_message = "您的版本有新的补丁包，请及时更新，补丁包版本(".$item['version']."-".$item['date'].")。下载地址：<a style='color:white' href='".$item['url']."' target='_blank'>".$item['url']."</a>";
            break;
        }
    }

    if (isset($update_message)) $smarty->assign('update_message', $update_message);
...
elseif ($_REQUEST['act'] == 'useabout')
{
    assign_query_info();
    $smarty->display('useabout.html');
}
/*------------------------------------------------------ */
//-- 在线授权
/*------------------------------------------------------ */

elseif ($_REQUEST['act'] == 'online_authorization')
{
    assign_query_info();
    $smarty->display('online_authorization.html');
}
