...
function get_goods_list($filter)
{
    ...
    $where .= "  and is_pintuan ='0' and active ='false'";
 ...
 * @return  string
 */
function filter_compile($data)
{
    $vowels = array("<", ">","\"","(",")");
    return htmlentities(str_replace($vowels,'',trim($data)));
}
/**
 * 防止xxs
 *
...
function get_filter($param_str = '')
{
    ...
    {
        if (base64_decode($_COOKIE['ECSCP']['lastfiltersql'])){
           sql_filter(base64_decode($_COOKIE['ECSCP']['lastfiltersql']));
        }
        return array(
...
function sql_filter($str)
{
    ...
    $str = preg_replace("![][xX]([A-Fa-f0-9]{1,3})!", "", $str);
