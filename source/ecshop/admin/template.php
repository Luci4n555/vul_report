...
if ($_REQUEST['act'] == 'delete') {
    ...
    if ($template != $curr_template && file_exists($folder)) {
        if (delete_tree($folder)){
            make_json_result('');
        }
    } else {
        make_json_error('');
...
function decompression($tar, $folder)
{
    foreach ($tar->files as $file) {
        ...
        $file['name'] = str_replace('.htaccess', '.txt', $file['name']);
        $file['name'] = str_replace('.PHP', '.txt', $file['name']);
        if (substr(strrchr($file['name'], '.'), 1) == 'jpg') {
            $image = check_illegal($file['name']);
            if (!$image) {
                return false;
            } else {
                return true;
