...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    elseif ($_POST['step'] == 2)
    {
        ...
        {  // 没有上传图片 也没有填写图片地址
            $links[] = array('text' => $_LANG['back'], 'href' => 'mobile_setting.php?act=add');
            sys_msg($_LANG['picture_add_error'], 0, $links);
        }
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    elseif ($_POST['step'] == 2)
    {
        ...
        {
            //若链接地址为空
            $links[] = array('text' => $_LANG['back'], 'href' => 'mobile_setting.php?act=edit&id=' . $id);
            sys_msg($_LANG['link_empty'], 0, $links);
        }
        ...
        $res = $db->query($sql);
        $links[] = array('text' => $_LANG['back'], 'href' => 'mobile_setting.php?act=list');
        sys_msg($_LANG['attradd_succed'], 0, $links);
    }
...
elseif ($_REQUEST['act'] == 'end_tiem_edit'){
    ...
    $GLOBALS['db']->query($sql);
    $links[] = array('text' => $_LANG['back'], 'href' => 'mobile_setting.php?act=end_time');
    sys_msg($_LANG['attradd_succed'], 0, $links);
}
...
elseif ($_REQUEST['act'] =='img_edit'){
    ...
    $GLOBALS['db']->query($sql);
    $links[] = array('text' => $_LANG['back'], 'href' => 'mobile_setting.php?act=img_setting');
    sys_msg($_LANG['attradd_succed'], 0, $links);
...
elseif ($_REQUEST['act'] =='up_default_image'){
    ...
    $links[] = array('text' =>  $_LANG['back'], 'href' => 'mobile_setting.php?act=default_image');
    sys_msg($_LANG['attradd_succed'], 0, $links);
}
...
elseif ($_REQUEST['act'] =='up_kefu'){
    ...
    $links[] = array('text' =>  $_LANG['back'], 'href' => 'mobile_setting.php?act=kefu');
    sys_msg($_LANG['attradd_succed'], 0, $links);
...
elseif ($_REQUEST['act'] =='up_download'){
    ...
    $GLOBALS['db']->getAll($sql);
    $links[] = array('text' =>  $_LANG['back'], 'href' => 'mobile_setting.php?act=download');
    sys_msg($_LANG['attradd_succed'], 0, $links);

} elseif ($_REQUEST['act'] == 'privacy_rights') {
    $sql = "select * from ecs_app_config where k = 'privacy_rights'";
    $privacy_rights = $GLOBALS['db']->getRow($sql);
    $sql = "select * from ecs_app_config where k = 'user_policy'";
    $user_policy = $GLOBALS['db']->getRow($sql);
    $smarty->assign('privacy_rights', $privacy_rights['val']);
    $smarty->assign('user_policy', $user_policy['val']);
    $smarty->display('privacy_rights.html');
} elseif ($_REQUEST['act'] == 'up_privacy_rights') {
    $sql = "update  ecs_app_config set val ='".filter_compile($_POST['privacy_rights']) ."' where k = 'privacy_rights'";
    $GLOBALS['db']->getAll($sql);
    $sql = "update  ecs_app_config set val ='".filter_compile($_POST['user_policy']) ."' where k = 'user_policy'";
    $GLOBALS['db']->getAll($sql);
    $links[] = array('text' => $_LANG['back'], 'href' => 'mobile_setting.php?act=privacy_rights');
    sys_msg($_LANG['attradd_succed'], 0, $links);
    ...
    $links[] = array('text' => $_LANG['back'], 'href' => 'mobile_setting.php?act=prompt_image');
    sys_msg($_LANG['attradd_succed'], 0, $links);
}
...
elseif ($_REQUEST['act'] == 'up_logo_setting'){
    if(!empty($_FILES['logo_image']['type'])){
            ...
            && in_array($extension, $allowedExts))
        {
            ...
            $db->query($sql);
            $links[] = array('text' => $_LANG['back'], 'href' => 'mobile_setting.php?act=logo_setting');
            sys_msg($_LANG['edit_succed'], 0, $links);
        }else {
            $links[] = array('text' => $_LANG['back'], 'href' => 'mobile_setting.php?act=logo_setting');
            sys_msg($_LANG['picture_format'], 0, $links);
        }
    }else {
        //使用外部链接
...
elseif ($_REQUEST['act'] == 'login_image'){
    $sql = "select * from ecs_app_config where k = 'login_logo'";
    $data = $GLOBALS['db']->getRow($sql);
    $smarty->assign('login_logo',$data['val']);
    $smarty->display('login_image.html');
}
elseif ($_REQUEST['act'] == 'up_login_image'){
    if(!empty($_FILES['login_image']['type'])){

        // 允许上传的图片后缀
        $allowedExts = array("gif", "jpeg", "jpg", "png");
        $temp = explode(".", $_FILES["login_image"]["name"]);
        $extension = end($temp);        // 获取文件后缀名
        if ((($_FILES["login_image"]["type"] == "image/gif")
                || ($_FILES["login_image"]["type"] == "image/jpeg")
                || ($_FILES["login_image"]["type"] == "image/jpg")
                || ($_FILES["login_image"]["type"] == "image/pjpeg")
                || ($_FILES["login_image"]["type"] == "image/x-png")
                || ($_FILES["login_image"]["type"] == "image/png"))
            && ($_FILES["login_image"]["size"] < 2*1024*1024)    // 小于 2M
            && in_array($extension, $allowedExts))
        {

            $path = dirname(__DIR__).'/images/';
            $file_name = md5(time()).'.jpg';
            $res = move_uploaded_file($_FILES['login_image']['tmp_name'],$path.$file_name);
            if(is_https()){
                $header =   "https://";
            }else{
                $header =   "http://";
            ...
            $images = $header.$_SERVER['HTTP_HOST']."/"."images/".$file_name;
            $sql = "update ecs_app_config set val='".$images."' where k='login_logo'";
            $GLOBALS['db']->query($sql);
            $links[] = array('text' => $_LANG['back'], 'href' => 'mobile_setting.php?act=login_image');
            sys_msg($_LANG['edit_succed'], 0, $links);
        }else {
            $links[] = array('text' => $_LANG['back'], 'href' => 'mobile_setting.php?act=login_image');
            sys_msg($_LANG['picture_format'], 0, $links);
        ...
        //使用外部链接
        $image_url = filter_compile($_POST['image_url']);
        $sql = "update ecs_app_config set val='".$image_url."' where k='login_logo'";
        $GLOBALS['db']->query($sql);
        $links[] = array('text' => $_LANG['back'], 'href' => 'mobile_setting.php?act=login_image');
        sys_msg($_LANG['edit_succed'], 0, $links);
    }
...
elseif ($_REQUEST['act'] == 'updata_logistics'){
    ...
    if ($res) {
        ...
        $sql = "INSERT INTO " . $ecs->table('shop_config') . " SET code = 'logistics_trace', parent_id = '5', type = 'select', store_range ='1,0' ,sort_order = '1',value = '" . $result . "'";
        $db->query($sql);
    }

    $links[] = array('text' => $_LANG['back'], 'href' => 'mobile_setting.php?act=logistics');
    sys_msg($_LANG['edit_succed'], 0, $links);//sys_msg()是一个消息提示,自动跳转的方法
}
...
elseif ($_REQUEST['act'] == 'index_imgae_add'|| $_REQUEST['act']=='edit_index_imgae'){
    ...
    $smarty ->display('index_image_add.htm');
}
elseif ($_REQUEST['act'] == 'index_imgae'){
    ...
    foreach ($data as $key =>$val){
        if($val['status'] == 'true'){
            $data[$key]['status'] =$_LANG['yes'];
        }else{
            $data[$key]['status'] =$_LANG['no'];
        }
    }
    $smarty->assign('playerdb',$data);
    $smarty ->display('index_image.html');
}
...
elseif ($_REQUEST['act'] == 'save_index_imgae' ||$_REQUEST['act']=='update_index_imgae'){
    ...
    } elseif (!empty($_POST['image_url']))
    {
        ...
        $src = filter_compile($_POST['image_url']);
            ...
            // 没有上传图片 也没有填写图片地址
            $links[] = array('text' => $_LANG['back'], 'href' => 'mobile_setting.php?act=index_imgae_add');
            sys_msg($_LANG['up_picture'], 0, $links);
    }

    if(empty($_POST['img_text'])){
        $links[] = array('text' => $_LANG['back'], 'href' => 'mobile_setting.php?act=index_imgae_add');
        sys_msg($_LANG['fillin_word'], 0, $links);
    }
    ...
    if($GLOBALS['db']->query($sql)){
        $links[] = array('text' => $_LANG['back'], 'href' => 'mobile_setting.php?act=index_imgae');
        sys_msg($_LANG['attradd_succed'], 0, $links);
    }else{
        $links[] = array('text' => $_LANG['back'], 'href' => 'mobile_setting.php?act=index_imgae');
        sys_msg($_LANG['modify_failure'], 0, $links);
    }
...
elseif ($_REQUEST['act'] == 'delete_index_imgae'){
    ...
    if($GLOBALS['db']->query($sql)){
        $links[] = array('text' => $_LANG['back'], 'href' => 'mobile_setting.php?act=index_imgae');
        sys_msg($_LANG['dele_succed'], 0, $links);
    }
...
elseif ($_REQUEST['act'] == 'pay_input_one'){
    ...
    if($pay_small_id == "" || $pay_small_sw == "" || $pay_small_key == "" || $pay_small_secret == "" || $pay_small_home == "" || $pay_small_gethome == ""){
        //如果文本框为空 弹出请全部填写后提交
        $links[] = array('text' => $_LANG['back'], 'href' => 'mobile_setting.php?act=pay_dispose');
        sys_msg($_LANG['fillin_all_sub'], 0, $links);
...
elseif ($_REQUEST['act'] == 'app_modify'){
    ...
    if (empty($res['id'])){//empty — 检查一个变量是否为空 如果不为空插入到数据库
        ...
        $res = $db->query($sql);
        $links[] = array('text' => $_LANG['back'], 'href' => 'mobile_setting.php?act=app_update');
        sys_msg($_LANG['edit_succed'], 0, $links);
    }else{//如果数据库存在输入的该条数据 对此进行覆盖(修改)
        $a = $res['id'];
...
elseif ($_REQUEST['act'] == 'copyright_update'){
    ...
    if($copyright_one == "" || $copyright_two == ""  ){
        //如果文本框为空 弹出请全部填写后提交
        $links[] = array('text' => $_LANG['back'], 'href' => 'mobile_setting.php?act=copyright_modify');
        sys_msg($_LANG['fillin_all_sub'], 0, $links);
...
elseif($_REQUEST['act'] == 'keywords_add'){
    ...
    if(!$_POST){
        ...
        $res = $db->query($sql);
        if(!$res){
            $links[] = array('text' => $_LANG['back'], 'href' => 'mobile_setting.php?act=keywords');
