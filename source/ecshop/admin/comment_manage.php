...
if ($_REQUEST['act'] == 'list')
{
    ...
    foreach ($list['item'] as $key => $value) {
        $list['item'][$key]['x_user_name'] = substr_cut($list['item'][$key]['user_name']);
    }
...
if ($_REQUEST['act']=='reply')
{
    ...
    $comment_info['x_user_name'] = substr_cut($comment_info['user_name']);
...
if ($_REQUEST['act']=='action')
{
    admin_priv('comment_priv');
    //获取回复的内容
    $reply_comment = filter_compile($_POST['content']);
    /* 获取IP地址 */
    $ip     = real_ip();
    ...
    else
    {
        ...
        $sql = "INSERT INTO ".$ecs->table('comment')." (comment_type, id_value, email, user_name , ".
               ...
               "VALUES('$_POST[comment_type]', '$_POST[id_value]','$_POST[email]', " .
                    "'$_SESSION[admin_name]','$reply_comment','" . gmtime() . "', '$ip', '1', '$_POST[comment_id]')";
    }
    $db->query($sql);
