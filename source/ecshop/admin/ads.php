...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $ad_name = !empty($_POST['ad_name']) ? filter_compile($_POST['ad_name']) : '';
    ...
    if ($_POST['media_type'] == '0')
    {
        ...
        {
            $ad_code = compile_str($_POST['img_url']);
        }
    ...
    $link_man = !empty($_POST['link_man']) ? filter_compile($_POST['link_man']) : '';
    $link_phone = !empty($_POST['link_phone']) ? filter_compile($_POST['link_phone']) : '';
    $link_email = !empty($_POST['link_email']) ? filter_compile($_POST['link_email']) : '';
    /* 插入数据 */
    $sql = "INSERT INTO ".$ecs->table('ad'). " (position_id,media_type,ad_name,ad_link,ad_code,start_time,end_time,link_man,link_email,link_phone,click_count,enabled,is_show)
...
elseif ($_REQUEST['act'] == 'update')
{
   ...
   sys_msg($_LANG['edit'] .' '.$ad_name.' '. $_LANG['attradd_succed'], 0, $href);
...
//  $filter['sort_order'] = empty($_REQUEST['sort_order']) ? 'DESC' : trim($_REQUEST['sort_order']);
    $filter['sort_by'] = empty($_REQUEST['sort_by']) ? 'ad.ad_name' : mysqli_real_escape_string($db->link_id, trim($_REQUEST['sort_by']));
    ...
    if (strpos($filter['sort_order'], 'updatexml') !== false) return false;
