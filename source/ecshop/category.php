...
$default_sort_order_type   = $_CFG['sort_order_type'] == '0' ? 'sort_order' : ($_CFG['sort_order_type'] == '1' ? 'shop_price' : 'last_update');//goods_id换为sort_order -生鲜

$sort  = (isset($_REQUEST['sort'])  && in_array(trim(strtolower($_REQUEST['sort'])), array('sort_order', 'shop_price', 'last_update', 'click_count')))? trim($_REQUEST['sort'])  : $default_sort_order_type;//array后goods_id换为sort_order,增加click_count -生鲜
...
if (!$smarty->is_cached('category.dwt', $cache_id))
{
    ...
    //$smarty->assign('categories',       get_categories_tree($cat_id)); // 分类树
	 $smarty->assign('categories', get_categories_tree()); // 显示全部分类树 -生鲜
    ...
    //新增分类页商品相册 -生鲜
	$smarty->assign('cat_name',   $cat['cat_name']);//-生鲜 分类名称
		/********************************************************************************************/  
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
              ...
              //-生鲜   " IFNULL(AVG(r.comment_rank),0) AS comment_rank,IF(r.comment_rank,count(*),0) AS  comment_count, " .
                'g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb , g.goods_img ' .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' .
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
        //-生鲜 ' LEFT JOIN  ' . $GLOBALS['ecs']->table('comment') . ' AS r ' .
       //-生鲜 'ON r.id_value = g.goods_id AND comment_type = 0 AND r.parent_id = 0 AND r.status = 1 ' .
       //-生鲜    "WHERE $where $ext group by g.goods_id ORDER BY $sort $order";
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$arr[$row['goods_id']]['buysum']          = ec_buysum($row['goods_id'])+get_cum_sales($row['goods_id']); //-生鲜 已销售数量	
	    $arr[$row['goods_id']]['evaluation']   = get_evaluation_sum($row['goods_id']);/*-生鲜，评价数===调用get_evaluation_sum*/
        //-生鲜 $arr[$row['goods_id']]['comment_rank'] = $row['comment_rank'] * 14;
        //-生鲜 $arr[$row['goods_id']]['comment_count'] = $row['comment_count'];
