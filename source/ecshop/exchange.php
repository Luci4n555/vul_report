...
function exchange_get_goods($children, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    /* 获得商品列表 */
    $sql = 'SELECT g.goods_id, g.goods_name,g.shop_price,g.goods_name_style, eg.exchange_integral, ' .
                'g.goods_type, g.goods_brief, g.goods_thumb , g.goods_img, eg.is_hot ' .
            'FROM ' . $GLOBALS['ecs']->table('exchange_goods') . ' AS eg, ' .$GLOBALS['ecs']->table('goods') . ' AS g ' .
            "WHERE eg.goods_id = g.goods_id AND $where $ext ORDER BY $sort $order";//增加g.shop_price -生鲜
...
//        else
		...
		$arr[$row['goods_id']]['shop_price']        = $row['shop_price'];//增加shop_price -生鲜
...
function get_exchange_recommend_goods($type = '', $cats = '', $min =0,  $max = 0, $ext='')
{
    ...
    $sql =  'SELECT g.goods_id, g.goods_name,g.shop_price, g.goods_name_style, eg.exchange_integral, ' .
            ...
            'WHERE eg.is_exchange = 1 AND g.is_delete = 0 ' . $price_where . $ext;///增加shop_price -生鲜
    $num = 0;
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$goods[$idx]['shop_price']        = $row['shop_price'];//增加shop_price -生鲜
